...
public class AiDetectionController {
    ...
    public void setData(User viewerUser, User patient, JournalAnalyseRow row) {
        ...
        this.row = row;
        titleLabel.setText("Detection AI - " + patientName());
        metaLabel.setText(safe(row.getDateJournal()) + " | Humeur: " + safe(row.getHumeur())
                + " | Detection locale: " + safe(row.getRisqueLabel()) + " (" + row.getRisqueScore() + ")");
    ...
    private void runDetection() {
        ...
        statusLabel.setText("Detection AI en cours...");
        resultTextArea.setText("");
        new Thread(() -> {
            AiJournalAnalysisService.AiResult result = aiService.detectRisk(patient, row);
            Platform.runLater(() -> {
