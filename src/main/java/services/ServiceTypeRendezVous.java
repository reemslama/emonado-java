...
public class ServiceTypeRendezVous {
    ...
    public boolean ajouter(TypeRendezVous typeRendezVous) {
        String sql = "INSERT INTO type_rendez_vous(libelle) VALUES (?)";
        try {
            Connection cnx = getConnection();
            PreparedStatement ps = cnx.prepareStatement(sql);
            ps.setString(1, typeRendezVous.getLibelle());
    ...
    public boolean modifier(TypeRendezVous typeRendezVous) {
        String sql = "UPDATE type_rendez_vous SET libelle = ? WHERE id = ?";
        try {
            Connection cnx = getConnection();
            PreparedStatement ps = cnx.prepareStatement(sql);
            ps.setString(1, typeRendezVous.getLibelle());
            ps.setInt(2, typeRendezVous.getId());
            return ps.executeUpdate() > 0;
        } catch (Exception e) {
    ...
    public List<TypeRendezVous> afficherTout() {
        ...
        try {
            ...
            while (rs.next()) {
                list.add(new TypeRendezVous(rs.getInt("id"), rs.getString("libelle")));
            }
        } catch (Exception e) {
    ...
    private void ensureTypeExists(String libelle) {
        ...
        try {
            Connection cnx = getConnection();
            PreparedStatement ps = cnx.prepareStatement("INSERT INTO type_rendez_vous(libelle) VALUES (?)");
            ps.setString(1, libelle);
            ps.executeUpdate();
        } catch (Exception e) {
