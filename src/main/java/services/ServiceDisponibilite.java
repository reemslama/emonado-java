...
public class ServiceDisponibilite {
    ...
    public List<Disponibilite> getDisposLibresByPsychologueAndDate(int psychologueId, LocalDate date) {
        ...
        String sql = "SELECT d.*, CONCAT(u.prenom, ' ', u.nom) AS psychologue_nom "
                + "FROM disponibilite d "
                + "LEFT JOIN user u ON u.id = d.psychologue_id "
                + "WHERE d.psychologue_id = ? AND d.date = ? AND d.est_libre = 1 AND TIMESTAMP(d.date, d.heure_debut) > NOW() "
        ...
        try {
            ...
            System.out.println("Erreur select dispo par date:");
            e.printStackTrace();
        }
        return list;
    }
    ...
    public boolean existeChevauchement(Disponibilite disponibilite, Integer excludedId) {
        String sql = "SELECT COUNT(*) FROM disponibilite WHERE psychologue_id = ? AND date = ? AND heure_debut < ? AND heure_fin > ?";
    ...
    public boolean estDisponiblePourReservation(int dispoId) {
        String sql = "SELECT COUNT(*) FROM disponibilite WHERE id = ? AND est_libre = 1 AND TIMESTAMP(date, heure_debut) > NOW()";
        try {
            Connection cnx = getConnection();
            PreparedStatement ps = cnx.prepareStatement(sql);
            ps.setInt(1, dispoId);
    ...
    private void fillPsychologueIdIfMissing() throws SQLException {
        Connection cnx = getConnection();
        String fallbackSql =
                "UPDATE disponibilite SET psychologue_id = (SELECT id FROM user WHERE role = 'ROLE_PSYCHOLOGUE' ORDER BY id LIMIT 1) WHERE psychologue_id IS NULL";
